...
(function(ns) {
    ...
    ns.MainScene = tm.createClass({
        ...
        init : function(settingData) {
            ...
            // selectSceneからデータを受け取る
            if (settingData) {
                TIMER = settingData.speed || TIMER;
                TIMER_TO_MILLISECOND = TIMER / 30 * 1000;
            }
            ...
            for (var i = 0; i < 9; ++i) {
                ...
                button.addEventListener("pointingend", function(e) {
                    // 入力を受付たら次の問題に移る
                    e.app.currentScene.animationLabel.tweener.rewind();

                    // バーも連動するようにリセットする
                    e.app.currentScene.bar.reset(TIMER);

                    // 回答を保存する
                    answer.push(this.label.text);
                });
                // グループに追加
